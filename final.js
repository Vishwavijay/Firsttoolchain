...
//Initialize Cloudant with settings from .env properties
var cloudant = Cloudant(
		...
		function(error) {
			if (error) {
				logger
						.info("Could not connect to Cloudant , Please check the Credentials");
...
exports.evscleanup = function() {
	console.log("inside evscleanup");

	// Use database
	// Assumes the database exists
	logger.info("Using database");
	var cloudantDB = cloudant.db.use("testemailverify");
	...
	cloudantDB
			.view(
					...
					function(err, body) {
						if (!err) {
							logger
									.info("Total count of the records for email verify deletion are:-------   "
											+ body.rows.length);
							console.log("just before printing from Cloudant" + body.rows.length);
	...
	logger.info("Batch Job of email verify is completed for " + limit_days
			+ " days, till " + end_date
			+ " Please check logentries.com emailverify for details.");
